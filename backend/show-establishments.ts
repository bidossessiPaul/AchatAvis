...
async function showEstablishments() {
    try {
        const results = await query('SELECT * FROM establishments');

        console.log('\n═══════════════════════════════════════════════════════════════');
        console.log(`📊 TOTAL ÉTABLISSEMENTS: ${results.length}`);
        console.log('═══════════════════════════════════════════════════════════════\n');

        results.forEach((est: any, index: number) => {
            ...
            if (est.platform_links && typeof est.platform_links === 'object') {
                console.log(`  • Platform Links: ${JSON.stringify(est.platform_links)}`);
            } else {
